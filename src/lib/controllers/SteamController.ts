...
export class SteamController {
   ...
   * ? Logging complete.
   */
  private static async cacheLogoConfigs(
    logoConfigs: fs.FileEntry[]
  ): Promise<void> {
    const configs = {};

    for (const logoConfig of logoConfigs) {
      const id = parseInt(
        logoConfig.name.substring(0, logoConfig.name.lastIndexOf("."))
      );
    ...
    LogController.log(
      `Cached logo positions for ${Object.entries(configs).length} games.`
    );
  }
   ...
   * ? Logging complete.
   */
  private static filterGridsDir(
    gridsDirContents: fs.FileEntry[],
    shortcutsIds: string[]
  ): [{ [appid: string]: LibraryCacheEntry }, fs.FileEntry[]] {
    let resKeys = [];
    const logoConfigs = [];
      ...
      if (fileEntry.name.endsWith(".json")) {
        ...
        if (foundApps.includes(idTypeString)) {
          ToastController.showWarningToast(
            `Duplicate grid found. Try cleaning`
          );
          ...
          if (
            gridTypeLUT[type] &&
            (type !== "icon" || shortcutsIds.includes(appid))
          ) {
   ...
   * ? Logging complete.
   */
  private static filterLibraryCache(
    libraryCacheContents: fs.FileEntry[],
    gridsInfos: { [appid: string]: LibraryCacheEntry },
    shortcutIds: string[]
    ...
    for (const fileEntry of libraryCacheContents) {
      ...
      const type = fileEntry.name.substring(
        firstUnderscore + 1,
        fileEntry.name.indexOf(".")
      );
      ...
      if (libraryCacheLUT[type]) {
        ...
        if (!Object.keys(res[appId]).includes(libraryCacheLUT[type]))
          res[appId][libraryCacheLUT[type]] = fileEntry.path;
    ...
    const filtered = entries.filter(
      ([appId, entry]) =>
        Object.keys(entry).length >= 2 || shortcutIds.includes(appId)
    ); //! Look into this because it seems like it aint ideal this because it caused issues with games with no grids
  ...
  static async getCacheData(
    shortcuts: GameStruct[]
  ): Promise<{ [appid: string]: LibraryCacheEntry }> {
    const gridsDir = await RustInterop.getGridsDirectory(
      get(activeUserId).toString()
    );
    const gridDirContents = await fs.readDir(gridsDir);

    const shortcutIds = Object.values(shortcuts).map((shortcut) =>
      shortcut.appid.toString()
    );
    const [filteredGrids, logoConfigs] = SteamController.filterGridsDir(
      gridDirContents,
      shortcutIds
    );
    ...
    if (libraryCacheDir.startsWith("DNE")) {
      await DialogController.message(
        "LibraryCache Did Not Exist!",
        "ERROR",
        `SARM was unable to read your librarycache folder. The path ${libraryCacheDir.substring(
          3
        )} did not exist. Please open Steam and go to the library tab so it caches a few game grids.`,
        "Ok"
      );
      await exit(0);
    }
    ...
    const filteredCache = SteamController.filterLibraryCache(
      libraryCacheContents,
      filteredGrids,
      shortcutIds
    );
   ...
   * ? Logging complete.
   */
  private static async getGamesFromSteamCommunity(
    bUserId: BigInt
  ): Promise<GameStruct[]> {
    const requestTimeout = get(requestTimeoutLength);
    LogController.log(`Loading games from Steam Community page...`);

    const res = await http.fetch<string>(
      `https://steamcommunity.com/profiles/${bUserId}/games?xml=1`,
      {
        method: "GET",
    ...
    if (res.ok) {
      const xmlData = SteamController.domParser.parseFromString(
        res.data,
        "text/xml"
      );
      ...
      return games.gamesList.games.game
        .map((game: any) => {
          return {
        ...
        .sort((gameA: GameStruct, gameB: GameStruct) =>
      ...
      ToastController.showWarningToast(
        "Error getting games from your profile."
      );
      LogController.warn(
        `Fetch Error: Status ${res.status}. Message: ${JSON.stringify(err)}.`
      );
      return [];
    }
  }
   ...
   * ? Logging complete.
   */
  private static async getGamesFromSteamAPI(
    bUserId: BigInt
  ): Promise<GameStruct[]> {
    const requestTimeout = get(requestTimeoutLength);
    LogController.log(`Loading games from Steam API...`);

    const res = await http.fetch<any>(
      `http://api.steampowered.com/IPlayerService/GetOwnedGames/v0001/?key=${get(
        steamKey
      )}&steamid=${bUserId}&format=json&include_appinfo=true&include_played_free_games=true`,
      {
        method: "GET",
        timeout: requestTimeout,
      }
    ...
    if (res.ok) {
      ...
      LogController.warn(
        `Fetch Error: Status ${res.status}. Message: ${JSON.stringify(
          err
        )}. User should check their Steam API Key.`
      );
      return [];
    }
  }
    ...
    return vdf.entries
      .map((game: any) => {
        return {
          appid: game.id,
          name:
            typeof game.common.name == "string"
              ? game.common.name.replace(/[^\x00-\x7F]/g, "")
              : game.common.name.toString(),
        } as GameStruct;
      })
      .sort((gameA: GameStruct, gameB: GameStruct) =>
        gameA.name.localeCompare(gameB.name)
  ...
  private static async getGamesFromLocalconfig(): Promise<GameStruct[]> {
    ...
    const localConfigContents = await RustInterop.readLocalconfigVdf(
      userId.toString()
    );
    return appInfoGames.filter((game) =>
      localConfigContents.includes(game.appid.toString())
    );
  }
  ...
  static async getUserApps(): Promise<void> {
    ...
    originalSteamShortcuts.set(
      JSON.parse(JSON.stringify(Object.values(shortcuts)))
    );
    steamShortcuts.set(Object.values(shortcuts));

    const structuredShortcuts = Object.values(shortcuts).map(
      (shortcut: any) => {
        return {
    ...
    const filteredCache =
      await SteamController.getCacheData(structuredShortcuts);
    ...
    if (online && !needsSteamAPIKey) {
      const apiGames = (await this.getGamesFromSteamAPI(bUserId)).filter(
        (entry) => filteredKeys.includes(entry.appid.toString())
      );
      ...
      try {
        const publicGames = (
          await this.getGamesFromSteamCommunity(bUserId)
        ).filter(
          (entry: GameStruct) =>
            filteredKeys.includes(entry.appid.toString()) &&
            !entry.name.toLowerCase().includes("soundtrack")
        );
        console.log("Public Games:", publicGames);
        steamGames.set(publicGames);

        LogController.log(
          `Loaded ${publicGames.length} games from Steam Community page.`
        );
        LogController.log("Steam games loaded.");
      } catch (err: any) {
        LogController.log(
          `Error occured while loading games from Steam Community page, notifying user.`
        );
        ToastController.showWarningToast("You profile is private");
        // TODO: consider prompting user here
        const appinfoGames = (await this.getGamesFromAppinfo()).filter(
          (entry: GameStruct) => filteredKeys.includes(entry.appid.toString())
        );
        console.log("Appinfo Games:", appinfoGames);
        steamGames.set(appinfoGames);

        LogController.log(
          `Loaded ${appinfoGames.length} games from appinfo.vdf.`
        );
        LogController.log("Steam games loaded.");
      }
    } else {
      const localconfigGames = (await this.getGamesFromLocalconfig()).filter(
        (entry: GameStruct) => filteredKeys.includes(entry.appid.toString())
      );
      ...
      LogController.log(
        `Loaded ${localconfigGames.length} games from localconfig.vdf.`
      );
      LogController.log("Steam games loaded.");
    }
    ...
    let manualGames = originalManualGames.filter((manualGame) => {
      const matchingSteamGame = sGames.find(
        (sGame) => sGame.appid == manualGame.appid
      );
      if (matchingSteamGame) {
        LogController.warn(
          `Found manually added game with the same appid (${manualGame.appid}) as ${matchingSteamGame.name}. Removing it`
        );
      }
    ...
    if (manualGames.length != originalManualGames.length) {
      ...
      ToastController.showWarningToast(
        `Removed ${Math.abs(
          manualGames.length - originalManualGames.length
        )} duplicate manual games!`
      );
    }
