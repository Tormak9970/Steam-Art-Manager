...
export class Reader {
   ...
   * @param data The data for this reader.
   */
  ...
  /**
   * Interface for reading data from the Reader's underlying buffer.
   * @param method The method to use to read the bytes.
   * @param bytes The number of bytes that will be read.
   * @returns The data read.
   */
  private readI(method: keyof DataView, bytes: number): (endianness?: boolean) => number {
   ...
   * @param position the position to update from. 0 = start, 1 = current offset, 2 = end.
   */
   ...
   * @param endianness whether or not to use littleEdian. Default is true.
   */
   ...
   * @param length optional length of the string. Reads until 00 byte if undefined.
   */
