...
export function filterGrids(
  ...
  const gridStyles = Object.keys(targetFilters.styles).filter(
    (style) => targetFilters.styles[style]
  );
  const dimensions =
    type != GridTypes.LOGO && type != GridTypes.ICON
      ? Object.keys(targetFilters.dimensions).filter(
          (dimension) => targetFilters.dimensions[dimension]
        )
      : [];
  const imageFormats = Object.keys(targetFilters.mimes).filter(
    (imgType) => targetFilters.mimes[imgType]
  );
  const animationTypes = Object.keys(targetFilters.types).filter(
    (gridType) => targetFilters.types[gridType]
  );
  ...
  const resGrids = allGrids.filter((grid: SGDBImage) => {
    return (
      gridStyles.includes(grid.style) &&
      (dimensions.includes(`${grid.width}x${grid.height}`) ||
        type == GridTypes.LOGO ||
        type == GridTypes.ICON) &&
      imageFormats.includes(grid.mime) &&
      (grid.isAnimated
        ? animationTypes.includes("animated")
        : animationTypes.includes("static")) &&
  ...
  if (resGrids.length > 0) {
    if (useCoreFile) {
      ...
      LogController.batchApplyLog(
        `Query: ${query}. Result: ${resGrids.length} grids.`
      );
    }
  } else {
    if (useCoreFile) {
...
export async function steamDialogSequence(): Promise<void> {
  return new Promise<void>(async (resolve) => {
    const hasSteamInstalled = await DialogController.ask(
      "Steam Not Found",
      "WARNING",
      "SARM could not locate Steam. Do you have it installed?",
      "Yes",
      "No"
    );
    ...
    if (hasSteamInstalled) {
      ...
      await DialogController.message(
        "SARM Could Not Initialize",
        "ERROR",
        "Please install Steam and login once, then restart SARM.",
        "Ok"
      );
      await exit(0);
      resolve();
...
export async function findSteamPath(savedInstallPath: string): Promise<void> {
  if (savedInstallPath !== "") {
    const steamInstallPathAdded =
      await RustInterop.addPathToScope(savedInstallPath);
    ...
    if (returnedInstallPath === "") {
      await DialogController.message(
        "Unrecoverable Error",
        "ERROR",
        "A Steam installation was found but could not be added to scope. Please restart, and if the problem persists, open an issue on SARM's GitHub repository.",
        "Ok"
      );
      await exit(0);
