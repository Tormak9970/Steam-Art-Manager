<script lang="ts">
  ...
  function onStateChange(
    ...
    gamesToFilter = (
      ...
      !showHidden ? !$hiddenGameIds.includes(game.appid) : true
    );
    const selectedGameEntries = gamesToFilter.map((game) => {
      return [
        game.appid,
        gameFilter == "All" ? true : !$appLibraryCache[game.appid][$gridType],
      ];
    });
  ...
  <div class="options">
    <div class="dropdowns">
      <DropDown
        ...
        width="100px"
        onChange="{(platform) => {
          onStateChange(platform, selectedGamesFilter, includeHidden);
        }}"
        showTooltip="{false}" />
      {#if showFilters}
        <DropDown
          ...
          onChange="{(gamesFilter) => {
            onStateChange(selectedPlatform, gamesFilter, includeHidden);
          }}"
          showTooltip="{false}" />
      {/if}
    </div>
    ...
    <Toggle
      ...
      onChange="{(showHidden) => {
        onStateChange(selectedPlatform, selectedGamesFilter, showHidden);
      }}" />
    <Spacer orientation="VERTICAL" />
  </div>
  <Table>
      ...
      {#each gamesToFilter as game, i (`${game.appid}|${i}`)}
        <SelectedGameEntry
          ...
          platform="{selectedPlatform != 'All'
            ? selectedPlatform
            : allSteamGames.some((steamGame) => steamGame.appid == game.appid)
            ? Platforms.STEAM
            : Platforms.NON_STEAM}"
