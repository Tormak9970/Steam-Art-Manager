<script lang="ts">
  ...
  let tempManualGames: GameStruct[] = JSON.parse(
    JSON.stringify(originalManualGames)
  );
  ...
  function addNewGame(game: GameStruct): void {
    if (
      ...
      ToastController.showWarningToast(
        `Game with that appid already exists! Can't have duplicates.`
      );
    } else {
      ...
      canSave =
        JSON.parse(JSON.stringify(originalManualGames)) !=
        JSON.parse(JSON.stringify(tempManualGames));
    }
  }
  ...
  async function saveChanges() {
    ...
    originalAppLibraryCache.set(
      JSON.parse(JSON.stringify(originalAppLibCache))
    );
    ...
    ToastController.showSuccessToast(
      `Saved ${tempManualGames.length} manually added games.`
    );
...
<ModalBody title="{'Manage Manual Games'}" {onClose}>
  <div class="content">
    <div class="left">
      <div class="info">
      ...
      <div class="section-label" style="margin-left: 10px;">
        Your Manual Games
      </div>
      <Table>
        <span slot="header">
          ...
          <div
            class="exist-art-icon"
            style="margin-left: 18px; margin-right: 34px;"
            use:AppController.tippy="{{
            ...
            <svg
              ...
              <path fill="#305b79" d="M0 0h41v18H0V0z"></path>
              <path fill="#4787b4" d="M3 2h41v18H3V2z"></path>
              <path fill="#5fb4f0" d="M6 4h41v18H6V4z"></path>
              <path fill="#3a6e92" d="M9 6h41v18H9V6z"></path>
            </svg>
          </div>
        </span>
        <span slot="data">
