<script lang="ts">
  ...
  $: originalName = (
    $steamGames.find((game) => game.appid === $selectedGameAppId) ??
    $nonSteamGames.find((game) => game.appid === $selectedGameAppId)
  )?.name;
  ...
  async function onSgdbGameChange(id: string): Promise<void> {
    if (
      ...
      grids = filterGridsWrapper(
        await AppController.getSteamGridArt(
          $selectedGameAppId,
          $selectedResultPage,
          id,
          false
      ...
      numPages =
        $steamGridSearchCache[$selectedGameAppId]?.find(
  ...
  function setAvailableSgdbGamesOnStateChange(
    searchCache: { [appid: number]: SGDBGame[] },
    selectedAppId: number
  ): void {
    if (
      ($currentPlatform == Platforms.STEAM ||
        $currentPlatform == Platforms.NON_STEAM) &&
      $selectedGameName &&
      searchCache[selectedAppId]
    ) {
      ...
      numPages =
        searchCache[selectedAppId]?.find(
          (game) => game.id.toString() == $selectedSteamGridGameId
        )?.numResultPages ?? 3;
    }
  }
   ...
   */
  async function filterGridsOnStateChange(
    sgdbApiKey: string,
    online: boolean,
    selectedAppId: number,
    selectedGridType: GridTypes,
    resultsPage: number,
    filters: DBFilters,
    selectedSGDBId: string | null,
    isCustomName: boolean = false
  ): Promise<void> {
    if (online && sgdbApiKey != "" && selectedAppId != null) {
      const unfilteredGrids = await AppController.getSteamGridArt(
        selectedAppId,
        resultsPage,
        selectedSGDBId,
        isCustomName
  ...
  onMount(() => {
    ...
    manualGamesUnsub = manualSteamGames.subscribe((games) => {
      if (
        $selectedGameAppId &&
        !games.find((game) => game.appid === $selectedGameAppId)
      ) {
        isLoading = true;
        resetGridStores();
        isLoading = false;
    ...
    customGameNamesUnsub = customGameNames.subscribe(async (customNames) => {
      if (!skipUpdate) {
        if (customNames[$selectedGameAppId] && !hasCustomName) {
          ...
          await filterGridsOnStateChange(
            $steamGridDBKey,
            $isOnline,
            $selectedGameAppId,
            $gridType,
...
<Pane minSize="{20}">
  <div class="inner">
    ...
    <div
      ...
      <div
        style="margin-left: 6px; display: flex; justify-content: space-between;">
        <div style="display: flex;">
          {#if !windowWidth || windowWidth >= 1265}
            <DropDown
              label="Browsing"
              options="{availableSteamGridGames}"
              onChange="{onSgdbGameChange}"
              width="{'130px'}"
          ...
          <IconButton
            ...
            tooltipPosition="{'top'}"
            disabled="{$selectedGameAppId == null}"
            height="24px"
            width="24px">
            <svg
    ...
    <div
      class="content"
      style="height: calc(100% - 85px);position: relative; z-index: 1;">
      {#if $isOnline}
        {#if !$needsSGDBAPIKey}
          {#if $selectedGameAppId != null}
            <Pages
              ...
              bind:selected="{$selectedResultPage}">
              <PaddedScrollContainer
                ...
                {#if isLoading}
                  <div
                    class="game-grid"
                    style="--img-width: {widths[$gridType] +
                      padding}px; --img-height: {heights[$gridType] +
                      padding +
                      18}px;">
                ...
                {:else}
                  <div class="message">
                    No results for {$gridType == GridTypes.HERO
                      ? "Heroe"
                      : $gridType}s for "{$selectedGameName}".
                  </div>
                {/if}
              </PaddedScrollContainer>
            </Pages>
          {:else}
            <div class="message">Select a game to start managing your art!</div>
          {/if}
        {:else}
          <div class="message">Please set your API key to use SteamGridDB.</div>
        {/if}
      {:else}
        <div class="message">
