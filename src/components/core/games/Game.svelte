...
<div class="game" class:selected={$selectedGameAppId == game.appid} on:click={selectGame}>
  ...
  {#if hasCustomName}
    <div class="image-control icon-30" on:click|stopPropagation={() => { AppController.clearCustomNameForGame(game.appid); }} use:AppController.tippy={{ content: "Clear Custom Name", placement: "right", onShow: AppController.onTippyShow}}>
  ...
  {/if}
  {#if hasCustomArt}
    <div class="image-control" class:icon-30={!hasCustomName} class:icon-58={hasCustomName} on:click|stopPropagation={() => { AppController.clearCustomArtForGame(game.appid); }} use:AppController.tippy={{ content: "Clear Art", placement: "right", onShow: AppController.onTippyShow}}>
