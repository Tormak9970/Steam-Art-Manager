    ...
    if (libraryCache[game.appid]) {
      if (libraryCache[game.appid][type]) {
        ...
        if (libraryCache[game.appid][type] == "REMOVE") {
          imagePath = tauri.convertFileSrc(
            $unfilteredLibraryCache[game.appid][type]
          );
        } else {
...
<div
  class="game"
  class:selected="{$selectedGameAppId == game.appid}"
  on:click="{selectGame}">
  <div
    class="image-control icon-2"
    on:click|stopPropagation="{isHidden ? unHide : hide}"
  ...
  {#if hasCustomName}
    <div
      ...
      on:click|stopPropagation="{() => {
        AppController.clearCustomNameForGame(game.appid);
      }}"
      use:AppController.tippy="{{
  ...
  {#if hasCustomArt}
    <div
      class="image-control"
      class:icon-30="{!hasCustomName}"
      class:icon-58="{hasCustomName}"
      on:click|stopPropagation="{() => {
        AppController.clearCustomArtForGame(game.appid);
      }}"
      use:AppController.tippy="{{
        content: 'Clear Art',
        placement: 'right',
  ...
  <GridImage
    ...
    {showImage}
    missingMessage="No {$gridType} image for game" />
  <div
    class="name"
    use:AppController.tippy="{{
      content: game.name,
