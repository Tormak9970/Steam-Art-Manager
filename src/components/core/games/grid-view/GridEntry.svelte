...
<div class="game" class:selected={$selectedGameAppId === game.appid} on:click={selectGame}>
  <div class="image-control icon-2" on:click|stopPropagation={() => showAllGrids(game.appid)} use:AppController.tippy={{ content: "View Grids", placement: "right", onShow: AppController.onTippyShow }}>
    <AllGridsIcon />
  </div>
  <div class="image-control icon-30" on:click|stopPropagation={() => toggleHidden(!isHidden)} use:AppController.tippy={{ content: isHidden ? "Unhide" : "Hide", placement: "right", onShow: AppController.onTippyShow }}>
    {#if isHidden}
  ...
  {#if hasCustomName}
    <div class="image-control icon-58" on:click|stopPropagation={() => { AppController.clearCustomNameForGame(game.appid); }} use:AppController.tippy={{ content: "Clear Custom Name", placement: "right", onShow: AppController.onTippyShow }}>
  ...
  {/if}
  {#if hasCustomArt}
    <div class="image-control" class:icon-58={!hasCustomName} class:icon-86={hasCustomName} on:click|stopPropagation={() => { AppController.clearCustomArtForGame(game.appid); }} use:AppController.tippy={{ content: "Clear Art", placement: "right", onShow: AppController.onTippyShow }}>
  ...
  {/if}
  {#if canDiscard}
    <div class="image-control" class:icon-58={!hasCustomName && !hasCustomArt} class:icon-86={(hasCustomName || hasCustomArt) && !(hasCustomName && hasCustomArt)} class:icon-114={hasCustomName && hasCustomArt} on:click|stopPropagation={() => { AppController.discardChangesForGame(game.appid); }} use:AppController.tippy={{ content: "Discard Changes", placement: "right", onShow: AppController.onTippyShow }}>
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
