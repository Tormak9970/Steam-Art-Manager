...
pub fn get_id_from_grid_name(grid_name: &str) -> (String, String) {
    let dot_index: usize = grid_name
        .find(".")
        .expect("File should have had a file extension");
    let underscore_index_res = grid_name.find("_");
    let name: &str = &grid_name[0..dot_index];

    if underscore_index_res.is_some() {
        let underscore_index =
            underscore_index_res.expect("Should have been able to get underscore index.");
...
fn construct_grid_export_name(
    filename: &str,
    id: &String,
    grid_type: &String,
    platform: &str,
    id_name_map: &Map<String, Value>,
) -> String {
    let dot_index: usize = filename
        .find(".")
        .expect("File should have had a file extension");
    ...
    if platform == "nonsteam" && id_name_map.contains_key(id) {
        let shortcut_name_value: &Value = id_name_map
            .get(id)
            .expect("Shortcut name map should have contained shortcut id.");
        let shortcut_name: &str = shortcut_name_value
            .as_str()
            .expect("Should have been able to convert shortcut name value to str.");
...
fn get_import_grid_name(
    app_handle: &AppHandle,
    filename: &str,
    name_id_map: &Map<String, Value>,
    ...
    if filename.contains("__") {
        ...
        if platform == "nonsteam" && name_id_map.contains_key(file_core) {
            let shortcut_id_value: &Value = name_id_map
                .get(file_core)
                .expect("Should have been able to get shortcut id from name map");
            file_core = shortcut_id_value
                .as_str()
                .expect("Should have been able to convert shortcut name to id.");
        }
...
fn generate_grids_zip(
    app_handle: &AppHandle,
    grids_dir_path: PathBuf,
    zip_file_path: PathBuf,
    platform_id_map: &Map<String, Value>,
    id_name_map: &Map<String, Value>,
) -> bool {
    let grids_dir_contents = read_dir(grids_dir_path).unwrap();
    let zip_file: File = File::create(zip_file_path)
        .expect("File's directory should have existed since user picked it.");
    ...
    for dir_entry in grids_dir_contents {
        ...
        let contents: Vec<u8> =
            read(entry.path()).expect("Should have been able to read file, but couldn't.");
        ...
        let platform_value: &Value = platform_id_map
            .get(&id)
            .expect("Platform map should have contained game/shortcut id.");
        let platform: &str = platform_value
            .as_str()
            .expect("Should have been able to convert platform to string.");

        let modified_filename =
            construct_grid_export_name(filename_str, &id, &grid_type, platform, id_name_map);
...
fn set_grids_from_zip(
    ...
    let zip_file =
        File::open(zip_file_path).expect("File should have existed since user picked it.");
    let buffer_reader = BufReader::new(zip_file);
    let mut zip_reader = zip::ZipArchive::new(buffer_reader)
    ...
    for i in 0..zip_reader.len() {
        ...
        if zip_file.is_file() {
            ...
            let (platform, appid, adjusted_file_name) = get_import_grid_name(
                app_handle,
                mangled_name
                    .to_str()
            ...
            if platform == "nonsteam" && adjusted_file_name.contains("icon") {
                let dest_path_str: &str = dest_path
                    .to_str()
                    .expect("Should have been able to convert dest path to string.");
...
pub async fn export_grids_to_zip(
    app_handle: AppHandle,
    steam_path: String,
...
#[tauri::command]
/// Sets the users grids from a Grids zip file.
pub async fn import_grids_from_zip(
    app_handle: AppHandle,
    steam_path: String,
    steam_active_user_id: String,
    name_id_map: Map<String, Value>,
    ...
    if file_path.is_some() {
        ...
        logger::log_to_core_file(
            ...
            format!(
                ...
                zip_path
                    .to_str()
                    .expect("Should have been able to convert path to string.")
            )
            .as_str(),
            0,
        );
