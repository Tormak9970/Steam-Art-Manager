...
pub async fn clean_grids(
    ...
    let appids_arr: Vec<String> = serde_json::from_str(all_appids.as_str())
    ...
    if preset == String::from("clean") {
        for dir_entry in grids_dir_contents {
            ...
            if found_apps.contains_key(&id_type_str) {
                // ? There's a conflict
                let (other_filename, other_full_path) = found_apps
                    .get(&id_type_str)
                    .expect("Map should have contained the id_type_str.");

                conflicts.push(CleanConflicts {
                    fileAPath: other_full_path.to_owned(),
                    fileAName: other_filename.to_owned(),
                    fileBPath: String::from(full_file_path_str),
                    fileBName: String::from(filename_str),
                    appid: id.clone(),
                ...
                found_apps.insert(
                    id_type_str,
                    (String::from(filename_str), String::from(full_file_path_str)),
                );
            }
        }
    } else {
        let game_ids_arr: Vec<String> = serde_json::from_str(selected_game_ids.as_str())
            .expect("Should have been able to deserialize selected appids array.");
    ...
    return serde_json::to_string(&conflicts)
        .expect("Should have been able to serialize conflict array.");
}
