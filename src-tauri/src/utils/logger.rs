...
pub fn get_core_log_path(app_handle: &AppHandle) -> PathBuf {
    let app_log_dir: PathBuf = app_handle
        .to_owned()
        .path()
        .app_log_dir()
        .expect("Tried to resolve app log dir and failed.");
...
pub fn log_to_file(log_path: &PathBuf, message: &str, level: u8) {
    ...
    if log_file_res.is_ok() {
        ...
        if let Err(e) = writeln!(
            log_file,
            "[Steam Art Manager] [{hour}:{min}:{sec}] [{level_name}]: {message}"
        ) {
            eprintln!("Couldn't write to file: {}", e);
        }
    } else {
        panic!(
            "Error opening log file! Log file path: {}",
            log_path.display()
        );
    }
}
