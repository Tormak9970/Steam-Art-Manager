...
pub fn write_shortcuts_vdf(path: &PathBuf, data: Value) {
  
}

fn write(reader: &mut Reader) -> Value {
  reader.seek(1, 0);
...
fn write_entry_map(reader: &mut Reader) -> Value {
  let mut props = Map::new();
...
fn write_entry_field(reader: &mut Reader, field_type: u8) -> Value {
  match field_type {
    0x00 => { //? map
      return read_entry_map(reader);
    },
