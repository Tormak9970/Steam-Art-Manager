...
fn get_app_tiles_dir(app_handle: AppHandle) -> PathBuf {
    let data_dir: PathBuf = app_handle
        ...
        .path()
        .data_dir()
        .expect("User's data directory should have existed.");
...
fn get_appinfo_from_shortcut(
    ...
    if filename_str.ends_with(".url") {
        ...
        let file_contents = fs::read_to_string(shortcut_path)
            .expect("Should have been able to read steam shortcut file.");
        ...
        if file_contents.contains("steam://rungameid") {
            ...
            while current_line.is_some() {
                ...
                if line.starts_with("Icon=steam_icon_") {
                    ...
                    tile_path = format!(
                        "~/.local/share/icons/hicolor/ICON_SIZE/steam_icon_{}.png",
                        appid
                    );
                }
...
pub fn get_apps_with_tiles(app_handle: AppHandle) -> String {
    ...
    let app_tiles_dir_contents =
        fs::read_dir(app_tiles_dir).expect("Should have been able to read app tiles directory.");
...
fn write_app_tile(tile_path: String, icon_path: String) -> bool {
    ...
    if icon_path.ends_with(".ico") {
        ...
        let img = ImageReader::open(image_src)
            .unwrap()
            .decode()
...
fn write_app_tile(tile_path: String, icon_path: String) -> bool {
    ...
    let image_src = PathBuf::from(&icon_path);
    let img_data = ImageReader::open(image_src)
        .unwrap()
        .decode()
        .expect("Should have been able to decode the image.");
    ...
    for size in sizes {
        ...
        let sized_dest_str =
            tile_path.replace("ICON_SIZE", format!("{}x{}", size_str, size_str).as_str());
...
pub fn write_app_tiles(
    ...
    let tile_paths: HashMap<String, String> =
        serde_json::from_str(tile_paths_str.as_str()).unwrap();

    let mut failed_ids: String = String::from("[");

    for (appid, tile_path) in tile_paths {
        let icon_path = new_tiles
            .get(&appid)
            .expect(format!("new_tiles should have had key {}", appid).as_str());
