...
fn add_grid_file_to_cache(
    ...
    cache_data: &mut Map<String, Value>,
    logo_configs: &mut Vec<String>,
) {
    ...
    if grid_type_key.is_some()
        && (!grid_type.eq_ignore_ascii_case("icon") || shortcut_ids.contains(&app_id))
    {
...
async fn filter_grids_dir(
    ...
    for entry_res in contents {
        ...
        let file_type = entry
            .file_type()
            .expect("Couldn't get grid cache dir entry's file type.");
...
fn add_library_file_to_cache(
    file_path: PathBuf,
    app_id: &str,
    grid_type: &str,
    grid_cache_data: &mut Map<String, Value>,
    unfiltered_cache: &mut Map<String, Value>,
) {
    ...
    if grid_type_key.is_some() {
        ...
        let path_str = file_path
            .to_str()
            .expect("Couldn't convert logo_config path to str")
            .to_owned();
...
fn add_library_folder_to_cache(
    parent_path: PathBuf,
    entry: DirEntry,
    dirname: &str,
    grid_info: &GridInfo,
...
async fn filter_library_dir(
    ...
    if contents_res.is_err() {
        let err = contents_res.err().unwrap();
        let message = format!(
            "Error reading library directory. error: {}",
            err.to_string()
        );
    ...
    for entry_res in contents {
        ...
        let file_type = entry
            .file_type()
            .expect("Couldn't get library cache dir entry's file type.");
        let filename = entry.file_name().to_str().unwrap().to_string();
...
pub async fn get_cache_data(
    ...
    let unfiltered_cache =
        filter_library_dir(&app_handle, steam_path, &steam_apps, &mut grid_cache_data).await;
